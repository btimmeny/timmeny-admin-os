...
**Depends on:** [ADR-0001](./adr/ADR-0001-admin-os-coordination-layer.md), [50 — Architecture](./50-Architecture.md), [60 — Domain Model](./60-Domain-Model.md), [70 — Implementation Strategy](./70-Implementation-Strategy.md), [80 — Monday.com Architecture](./80-Monday-Architecture.md), [90 — Roadmap](./90-Roadmap.md)
...
external_mappings (
  ...
  admin_os_ref text unique,                   -- written into Monday for crash recovery
  created_at timestamptz, updated_at timestamptz,
...
Gmail API over user OAuth (installed-app flow, offline access, long-lived refresh token in Railway). A service account will not work — domain-wide delegation is unavailable for a personal `gmail.com` mailbox.

- Scope: `https://www.googleapis.com/auth/gmail.modify`. `gmail.readonly` cannot label or archive; `gmail.labels` alone cannot read.
- Dependencies: `google-auth`, `google-auth-oauthlib`, `google-api-python-client` (or raw `httpx` against the REST API to stay consistent with the Monday adapter — recommended, it is a small surface and avoids a heavy dependency tree).
- Sync: poll `users.threads.list` with a bounded query (strongly recommended: a dedicated intake label, not the whole inbox), then `users.threads.get` with `format=metadata` plus a bounded snippet.
- Archive: `users.threads.modify` with `removeLabelIds: ["INBOX"]`. Guarded by a `GMAIL_WRITE_ENABLED` flag so the first deployments are read-only.
- Idempotency: Gmail label operations are naturally idempotent; the thread id is the natural evidence key.
- Fallback if creating a Google Cloud OAuth client is undesirable: IMAP with an app password can archive, but it cannot manage Gmail labels cleanly and offers no message-id stability guarantees. Not recommended.
...
3. `find_item_by_admin_os_ref(board_id, ref)` — recovery lookup used before any retried create.
4. `create_item_idempotent(...)` — reserves an `external_mappings` row in `pending` with a generated `admin_os_ref`, writes the ref into a Monday column during creation, verifies, then marks the mapping `active`. A retry that finds a `pending` mapping searches by ref before creating.
...
Item 4 requires **one new text column on the target board** (proposed title: `Admin OS Ref`). This is a material decision and should be recorded as ADR-0002 before implementation.
...
- **Monday create.** Mapping row reserved *before* the write, `admin_os_ref` embedded in the created item, recovery search by ref before any retried create. This closes the crash-between-create-and-commit window that the Monday API cannot close on its own.
...
| `GMAIL_REFRESH_TOKEN` | offline access for `brian.timmeny@gmail.com`, scope `gmail.modify` | **needed** |
| `GMAIL_INTAKE_QUERY` or `GMAIL_INTAKE_LABEL` | bounds the evidence set | **decision needed** |
...
| `TODO_BOARD_ID` / `GS_TODO_BOARD_ID` | existing | slice target must be chosen |
| `ADMIN_OS_REF_COLUMN_TITLE` | mapping column title | new, requires a board column |
...
|---|---|---|
| 1 | This assessment, ADR-0002/0003 | approved |
...
1. *Monday create is not idempotent.* A crash between the mutation and the commit can duplicate a task. Mitigated by the reserved-mapping + `Admin OS Ref` recovery design, which needs a board column.
...
**Unknowns:** the real label set of the target board's `Status` column; the Monday board and group the slice should target; the intake query that defines "evidence"; whether Railway Postgres already exists on the account.
