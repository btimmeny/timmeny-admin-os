...
**Intake scope.** The service reads only threads that are **in the inbox and carry a configured capability label** — Gmail ANDs the two label ids, so both must hold. It does not read the inbox at large. The labels are configuration (`config/capabilities.yaml`, see [ADR-0007](ADR-0007-daily-review-engine.md)), so the intake set can be widened one label at a time as the loop proves reliable. Anything outside that intersection is invisible to Admin OS.
